...
namespace __WEB_API__TemplateFonctionnel_WebApi
{
    ...
    public class TemplateFonctionnelController : Controller
    {
        ...
        [HttpGet]
        [Route("DetailsByProject")]
        [ProducesResponseType(StatusCodes.Status200OK, Type = typeof(TemplateProject))]
        [ProducesResponseType(StatusCodes.Status400BadRequest)]
        public async Task<IActionResult> DetailsByProject(int id)
        {

            try
            {
                TemplateFonctionnel templateFonctionnel = await _itemplateFonctionnelService.DetailsByProject(id);
                TemplateFonctionnelVM templateFonctionnelVM = _mapper.Map<TemplateFonctionnelVM>(templateFonctionnel);
